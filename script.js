document.addEventListener('DOMContentLoaded', function() {
    ...
    // Determine the correct path for experience.json based on current page
    let experienceJsonPath = 'data/experience.json';
    if (window.location.pathname.includes('/pages/')) {
        // If we're in the pages directory, we need to go up one level
        experienceJsonPath = '../data/experience.json';
    }

    fetch(experienceJsonPath)
        .then(response => response.json())
        .then(data => {
            // If experience container exists on this page, render items
            const experienceContainer = document.getElementById('experience-container');
            if (experienceContainer) {
                renderExperienceItems(data, 'experience-container');
            }
            
            // If an all-experience page exists, render all items there too
            const allExperienceSection = document.getElementById('all-experience');
            if (allExperienceSection) {
                renderExperienceItems(data, 'all-experience-container');
            }
        })
        .catch(error => {
...
function renderExperienceItems(experienceData, containerId) {
    ...
    // Add each experience item to the container
    experienceData.forEach(item => {
        ...
        dateHighlight.textContent = item.date;
        dateElement.appendChild(dateHighlight);
        
        // Create the content element
        const contentElement = document.createElement('div');
        contentElement.className = 'experience-content';
        
        // Create the title element
        const titleElement = document.createElement('h3');
        
        // Allow HTML in title if present
        if (item.title && item.title.includes('<a href=')) {
        ...
        paragraphElement.innerHTML = item.content || '';
        ...
        // Optional right-side thumbnail image
        let thumbWrapper = null;
        if (item.image) {
            thumbWrapper = document.createElement('div');
            thumbWrapper.className = 'experience-thumb';
            const img = document.createElement('img');
            img.src = item.image;
            img.alt = item.imageAlt || item.title || 'experience image';
            thumbWrapper.appendChild(img);
        }
        
        // Add date, content and optional image to the experience item
        wrapper.appendChild(dateElement);
        wrapper.appendChild(contentElement);
        if (thumbWrapper) wrapper.appendChild(thumbWrapper);
        
        // Add the experience item to the container
